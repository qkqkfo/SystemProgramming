...
int inItO = 0;				//파일의 이름은 필수로 받아야하는 조건이므로 
							//받았는지 안받았는지 프로그램에게 알려주기 위해 선언
...
int cutFile(const char * pathName, const char * fileName, char * resultName, int BUFSIZE) {
	int inFile, outFile;										//파일 기술자
	ssize_t nread;												//nread의 크기만큼 읽기
	char * buffer = (char *)malloc(sizeof(char)*BUFSIZE);		//버퍼 크기만큼 동적할당
