...
int size = 512 >> 10;		//default size = 512KB
...
int cutFile(const char * pathName, const char * fileName, char * resultName, int BUFSIZE) {
	...
	while ((nread = read(inFile, buffer, BUFSIZE)) > 0) {
		...
		if ((outFile = open("%s%03d",resultName, resultFileNum++), O_WRONLY | O_CREAT | O_TRUNC, PERM)) == -1) {		//PERM = 0664 ( -rw-rw-r--)
			close(inFile);
			return -2;
		}
